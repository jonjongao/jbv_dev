<template>
  <div id="mainContainer">
    ...
    <pre v-for="p in 20" :key="p" style="height: 24px; display: inline-flex">
        <span v-if="isExist(p-1)"><div><span class="" data-type="bbsline" data-row="4"><span><span class="q7 b0 pointer">{{ isSelect(p-1) }}</span><span class="q7 b0 index">{{ getIndex(p-1) }}</span><span class="q9 b0 trend">{{ getTrend(getMETA[p-1].trend) }}</span><span class="q7 b0 name">{{ getName(getMETA[p-1].name) }}</span><span class="q7 b0 class">{{ getClass(getMETA[p-1].class) }}</span><span class="q7 b0 caption">{{ getMETA[p-1].caption }}</span><span class="q7 b0 pop">{{ getMETA[p-1].pop }}</span><span class="q7 b0 manager">{{ getMETA[p-1].manager }}</span></span></span><div></div></div></span>
        <span v-else style="width:970px"></span>
    </pre>
...
export default {
  ...
  computed: {
    getMETA: function () {
      //   console.log(
      //     u.getSpaceCount(
      //       "4 ˇMayDay       臺灣 ◎五月天。徵文活動按 Ｖ 投票中       7 shaneibalaja"
      //     )
      //   );
  ...
  methods: {
    ...
    getRandomASCIIColor: function () {
      var randomColor = Math.floor(Math.random() * 16777215).toString(16);
      return "#" + randomColor;
    },
    isExist: function (index) {
      console.log(index + " vs " + this.getMETA.length);
      if (index + 1 > this.getMETA.length) return false;
      else {
        console.log(index + " is pass");
        return true;
      }
    },
    getColor: function () {
